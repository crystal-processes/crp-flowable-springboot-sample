...
Test models comprehensively. Use [crp-flowable-assert](https://github.com/crystal-processes/crp-flowable-ex/tree/main/crp-flowable-assert) 
for flowable entities assertions.  
...
1. **Flowable variables are overused.** There are too many variables in the process instance. The variable types are complex
   (bytes, serializable, longString, jpa-entity-list). Try to propose JPA entities instead of too many or complex 
   process variables. 
...
5. **Long transactions.** The transaction starts on the event. The event examples: process started, user task completed,
